...
> For the prior full-history pass, see [`docs/g0-baseline-fidelity-gate-v0.5.md`](g0-baseline-fidelity-gate-v0.5.md). For the DC-RS/Reflexion follow-up that preceded this stricter release, see [`docs/g0-dc-rs-reflexion-fidelity-followup.md`](g0-dc-rs-reflexion-fidelity-followup.md).

---

## TL;DR

`v0.6` tightens the Reflexion same-sample retry semantics to the stricter Option A fidelity: the retry prompt no longer exposes the failed raw response, parsed answer, or verifier feedback directly. It conditions only on the latest three ordered reflection memories plus the current task input. The reflection stage still sees the failed trajectory and sanitized evaluator feedback so it can produce a mitigation memory, but that information reaches the retry actor exclusively through the reflection memory.

- Faithful adapted DC-RS optional appendix comparator remains unchanged from the post-`v0.5` follow-up.
- Faithful adapted Reflexion control flow now uses a strict memory boundary: failed-attempt details are visible only to `reflexion_reflect`, and `reflexion_generate` retries read only the latest-three reflections and the current input.
...
The gate uses the locked 3-task pilot set with three arms and two replay model labels, restricted to DC-RS and Reflexion:
...
| Failure-gated generation/reflection loop (Reflexion) | Reflection triggered only after verifier failure with sanitized reason; retry prompt does **not** repeat the failed trajectory | Success-only reflection, multi-step planning, and environment-specific scaffolding |
| Latest-three reflection memory conditions retry (Reflexion) | Ordered latest-three reflections rendered from the current identity; the retry actor sees **only** these reflections plus the current input | Weight updates, value functions, and external learning signals |
...
### reflexion_style — faithful adapted Reflexion control flow (v0.6 stricter boundary)
...
- With `max_attempts: 2`, a same-sample `reflexion_generate` retry runs using the latest-three ordered reflections from the current identity **plus the current task input only**. The retry prompt does not contain the failed raw response, parsed answer, or verifier feedback.
...
- This gate covers `dynamic_cheatsheet_rs_optional` and `reflexion_style` only. `no_memory`, `full_history`, `retrieval_rag`, `bot_style`, the historical `dynamic_cheatsheet_optional` DC-Cu baseline, and `expel_optional` are not claimed to pass this slice.
...
- `docs/g0-baseline-fidelity-gate-v0.6.md`
- `README.md`
...
The v0.5 faithful full-history gate, the v0.4 faithful RAG/BoT gate, and the v0.2 multitask replay demo still run without API keys. See [`docs/g0-baseline-fidelity-gate-v0.5.md`](g0-baseline-fidelity-gate-v0.5.md) for the full-history report, [`docs/g0-baseline-fidelity-gate-v0.4.md`](g0-baseline-fidelity-gate-v0.4.md) for the RAG/BoT report, and [`docs/g0-baseline-fidelity-gate-v0.2.md`](g0-baseline-fidelity-gate-v0.2.md) for the pre-implementation gap analysis.
